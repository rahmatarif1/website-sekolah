...
function KeuanganBos() {
  const [kataKunci, setKataKunci] = useState("");

  const dataBos = keuanganData.filter((k) => k.kategori === "BOS");
  const tersaring = dataBos.filter((item) =>
    item.judul.toLowerCase().includes(kataKunci.toLowerCase())
  );

  return (
    <div className="min-h-screen pt-20 md:pt-28 pb-12 md:pb-16">
      <Breadcrumb judul="BOS" induk={{ label: "Keuangan", to: "/keuangan/bos" }} />
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 mt-6">
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <aside className="md:col-span-1 space-y-8 order-2 md:order-1">
            ...
            {/* Category — navigasi antar halaman keuangan */}
            <div>
              ...
              <ul className="space-y-2 text-slate-600">
                <li className="border-b border-dashed border-slate-300 pb-2">
                  <Link to="/keuangan/bos" className="block hover:text-emerald-700 text-emerald-700 font-bold">BOS</Link>
                </li>
                <li>
                  <Link to="/keuangan/komite" className="block hover:text-emerald-700">Komite</Link>
                </li>
              </ul>
            </div>
