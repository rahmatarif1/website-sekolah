...
function Berita() {
  ...
  // SARING beritaData berdasarkan 3 filter
  const beritaTersaring = beritaData.filter((berita) => {
  const cocokKata = berita.judul.toLowerCase().includes(kataKunci.toLowerCase());
  const cocokTahun = tahunAktif === "Semua" || berita.tahun === Number(tahunAktif);
  return cocokKata && cocokTahun;
});

  return (
    <div className="min-h-screen pt-20 md:pt-28 pb-12 md:pb-16">
      <Breadcrumb judul="Berita" />
...
<div>
  ...
  <ul className="space-y-2 text-slate-600">
    <li className="border-b border-dashed border-slate-300 pb-2">
      <Link to="/berita" className="block hover:text-emerald-700 text-emerald-700 font-bold">Berita Sekolah</Link>
    </li>
    <li className="border-b border-dashed border-slate-300 pb-2">
      <Link to="/info-sekolah" className="block hover:text-emerald-700">Info Sekolah</Link>
    </li>
    <li>
      <Link to="/agenda" className="block hover:text-emerald-700">Agenda</Link>
          ...
          <div className="md:col-span-2 order-1 md:order-2">
            {beritaTersaring.length > 0 ? (
              ...
              <p className="text-slate-500">Tidak ada berita yang cocok.</p>
            )}
          </div>
