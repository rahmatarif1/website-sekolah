...
function Agenda() {
  ...
  const agendaTersaring = agendaData.filter((info) => {
...
<div>
  ...
  <ul className="space-y-2 text-slate-600">
    ...
    <li className="border-b border-dashed border-slate-300 pb-2">
     <Link to="/agenda" className="block hover:text-emerald-700">Info Sekolah</Link>
    </li>
    <li>
        <Link to="/info-sekolah" className="block hover:text-emerald-700 text-emerald-700 font-bold">Agenda</Link>
    </li>
  </ul>
</div>
            {/* Archives */}
...
<div className="md:col-span-2">
  {agendaTersaring.length > 0 ? (
    agendaTersaring.map((agenda) => (
      <KartuBerita key={agenda.id} berita={agenda} basePath="/agenda" />
    ))
  ) : (
    <p className="text-slate-500">Tidak ada agenda yang cocok.</p>
  )}
</div>
